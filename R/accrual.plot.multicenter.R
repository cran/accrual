accrual.plot.multicenter<-function(n,T,P,J,Tm,Tsj,m,all){
  nmax=2*n
  accrual.multi.out=accrual.multi.n(n,T,P,J,Tm,Tsj,m,Tpred=T,all)
  ...
  plot(c(-0.5,T),c(0,nmax),xlab="Time (months)",
  ...
  legendc=paste("Subjects recruited:",sum(m))
  no.site=dim(site.pred)[2]
  legende=paste("No of site for prediction:",no.site)
  legendf=paste("Subjects in", T, "months:",dist.n.pred[2],"(",round(dist.n.pred[1]),",",round(dist.n.pred[3]),")" )
  ...
  for (i in 1:length(tlist)){
    Ti=tlist[i]
    accrul.multi=quantile(accrual.multi.n(n,T,P,J,Tm,Tsj,m,Ti,all)[[2]],prob=c(0.025,0.5,0.975))
